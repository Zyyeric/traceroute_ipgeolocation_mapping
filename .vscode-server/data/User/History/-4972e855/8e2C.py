...
def calculate_radius(curr_min_rtt, prev_min_rtt):
    ...
    print(curr_min_rtt, prev_min_rtt)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # Write all data points to a JSON file
                all_data_filename = f"results/{base_filename}_all_data_points.json"
                with open(all_data_filename, 'w') as f:
                    json.dump(result['all_data_points'], f, indent=4)
                
                print(f"All data points written to: {all_data_filename}")
